...
const patrol = function (stateDescriptor) {
  ...
  go(function* () {
    while (true) {
      ...
      if (persister) {
        persister(setCh, path, convertToNative(getStateByPath(currentState, path)), changes.pop(), currentState);
      }
  ...
  go(function* () {
    while (true) {
      ...
      if (fetcher) {
        fetcher(setCh, path, convertToNative(getStateByPath(currentState, path)));
      }
    }
  });
